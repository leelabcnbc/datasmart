...
class TestFileTransferLocal(unittest.TestCase):
    ...
    def check_local_push_fetch_result(self, fetch_flag, ret, external_site,
        ...
        local_site_dict = {'local': True,
                           'path': datasmart.core.util.path.joinpath_norm(os.path.abspath(self.local_data_dir),
                                                                          *subdirs_this)}
        external_site_dict = {'local': True,
                              'path': datasmart.core.util.path.joinpath_norm(os.path.abspath(external_site))}
        if not fetch_flag:
        ...
        if not fetch_flag:
            self.assertEqual(ret['filelist'], [datasmart.core.util.path.joinpath_norm(dest_append_prefix,
